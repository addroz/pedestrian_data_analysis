...
forvalues s=1/`n_periods'{
	...
	twoway (scatter coef_outbreak time,  msymbol(D) lcolor(navy) mcolor(navy) ) (rcap ci_outbreak_high ci_outbreak_low time, lcolor(navy)), ///
			...
			xline(25.5, lwidth(19) lc(gs14))    yline(0, lcolor(black)) xline(22, lc(cranberry))  xlabel(`first_lead' "- `number_leads'" 8 "-14"   15 "-7"  `start_period'  "0" 29 "+7"  36 "+14" `last_lag' "+ `number_lags'" , labgap(*1.4)) ///
	...
	graph export "${figures}from_`s_start'_`s_num'h_window_region_fes.pdf" , as(pdf) replace 
	
	restore
}

// Same but without the weekends
forvalues s=1/`n_periods'{
	use "${statadata}06_Full_Data_Merged.dta", clear
	...
	graph export "${figures}from_`s_start'_`s_num'h_window_region_fes_no_weekends.pdf" , as(pdf) replace 
